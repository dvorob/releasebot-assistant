node('docker') {
    ansiColor('xterm') {
        ...
        def image = 'releasebot-assistant'
        def home_dir = '/var/lib/jenkins/workspace/Common/releasebot/assistant'
        ...
        try {
            stage('docker build && push') {
                ...
                checkout ([$class: 'GitSCM',
                    ...
                    url: 'ssh://git@bitbucket.yooteam.ru/admin-tools/releasebot-assistant.git']]])
            ...
            stage('helm lint') {
                docker.image("${dockerImageUrl}").inside() {
                    ...
                    helm install --dry-run releasebot-assistant . --kubeconfig ../ansible/kubeconfig.yml -f ../ansible/secret_values.yml -f ../tag-values.yaml -n releasebot
                    '''
                    notifyBitbucket(buildStatus: 'INPROGRESS')
                }
            }
            stage('helm install') {
                docker.image("${dockerImageUrl}").inside() {
                    sh '''
                    cd ./deploy &&
                    helm upgrade --install releasebot-assistant . --kubeconfig ../ansible/kubeconfig.yml -f ../ansible/secret_values.yml -f ../tag-values.yaml -n releasebot
                    '''
                }
            }
